...
struct InstrumentationView: View {
    ...
    var body: some View {
        VStack {
            GeometryReader { g in
                // Should only show compactContent on iPhone SE (or similar device size)
                if g.size.height <= 180.0 {
                    self.compactContent()
                } else {
                    self.normalContent()
                }
    ...
    func compactContent() -> some View {
        VStack {
            Spacer()
            // Credit for PageView comes from Majid Jabrayilov : https://swiftwithmajid.com/2019/12/25/building-pager-view-in-swiftui/
            PagerView(pageCount: 2, currentIndex: $pageIndex) {
                VStack{
                    control1(width)
